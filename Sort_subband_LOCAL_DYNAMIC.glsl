...
shared float input_b[SUBBAND_WIDTH];
...
void store_all_from_img_to_arr(int L_H_0, int L_H_1){
    ...
    vec4 pix_r = vec4(0.0, 0.0, 0.0, 1.0);
        ...
        //HH subband
        if(L_H_0 == 1 && L_H_1 == 1){
           pix_l = imageLoad(image_O, ivec2(th_id.x + SUBBAND_WIDTH, th_id.y + SUBBAND_WIDTH));
           pix_r = imageLoad(image_O, ivec2(th_id.x + SUBBAND_WIDTH + NUM_INV, th_id.y + SUBBAND_WIDTH));
        }

        //LH subband
        if(L_H_0 == 0 && L_H_1 == 1){
            pix_l = imageLoad(image_O, ivec2(th_id.x, th_id.y + SUBBAND_WIDTH));
            pix_r = imageLoad(image_O, ivec2(th_id.x + NUM_INV, th_id.y + SUBBAND_WIDTH));
        }

        //HL subband
        if(L_H_0 == 1 && L_H_1 == 0){
            pix_l = imageLoad(image_O, ivec2(th_id.x + SUBBAND_WIDTH, th_id.y));
            pix_r = imageLoad(image_O, ivec2(th_id.x + SUBBAND_WIDTH + NUM_INV, th_id.y));
        }
...
void swap_val(int left, int right){
    ...
    void bitonic_sort_1(int start, int end){
        ...
        for(int k = 2; k <= s/NUM_PARTITIONS; k *= 2){

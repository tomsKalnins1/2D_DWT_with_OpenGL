...
<IMAGE EXAMPLE OF DWT AND IDWT MATRIX TEXTURE>

It is clear that to perform the convolution needed there are many redundant multiplications done if the image and the transform are multiplied element by element. The newer version of <link to shaders for apply_DWT and apply_IDWT> implementation uses sparse convolution to avoid redundant operations.

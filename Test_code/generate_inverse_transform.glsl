...
layout(rgba32f, binding = 0) uniform image2D idwt_mat;
...
void main()
{
    ...
    if(ind_right >=0 && ind_right < 4){
     ...
     imageStore(idwt_mat, ivec2(w_id.x, 255 - th_id.y), pix_low_0);
     imageStore(idwt_mat, ivec2(w_id.x + 128, 255 - th_id.y), pix_low_1);
     
     imageStore(idwt_mat, ivec2(w_id.x, 127 - th_id.y), pix_high_0);
     imageStore(idwt_mat, ivec2(w_id.x + 128, 127 - th_id.y),pix_high_1);

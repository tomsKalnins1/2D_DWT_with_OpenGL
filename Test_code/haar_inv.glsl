...
void main()
{
    ...
    for (int k = 0; k < 4; k++)
    {
        vec4 pix = imageLoad(image_O, ivec2((w_id.x * 2 + k) % 256, 255 - texC_g.y));
        vec4 pix_wt_low = imageLoad(dwt_mat, ivec2((w_id.x * 2 + k) % 256, 255 - w_id.x));
        vec4 pix_wt_high = imageLoad(dwt_mat, ivec2((w_id.x * 2 + k) % 256, 127 - w_id.x));
        res_high = pix_wt_low.x;
        res_low = pix_wt_high.x;
        //       res_high =  pix.x;
    }
    imageStore(image_T, ivec2(w_id.x, 255 - texC_g.y), vec4(res_low, res_low, res_low, 1.0));
    imageStore(image_T, ivec2(w_id.x + 128, 255 - texC_g.y), vec4(res_high, res_high, res_high, 1.0));

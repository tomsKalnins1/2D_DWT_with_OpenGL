...
void main()
{
   ivec2 th_id = ivec2(gl_GlobalInvocationID.xy);
    ...
    pix_sum_r += imageLoad(LL, ivec2(th_id.x + 128, th_id.y));
    pix_sum_r += imageLoad(LH, ivec2(th_id.x + 128, th_id.y));
    pix_sum_r += imageLoad(HL, ivec2(th_id.x + 128, th_id.y));
    pix_sum_r += imageLoad(HH, ivec2(th_id.x + 128, th_id.y));

    imageStore(dwt, ivec2(th_id.x, th_id.y), pix_sum_l);
    imageStore(dwt, ivec2(th_id.x + 128, th_id.y), pix_sum_r);
